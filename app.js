...
const SEASON_META={
  current:'02/26',
  seasons:{'02/26':{label:'02/26',status:'active',type:'Offiziell · 1. Saison',official:true,rules:'DoG 02/26 · v1'}}
};
...
function save(){reconcileCurrentRoster();normalizeAllRacePoints();localStorage.setItem('dogrh_drivers',JSON.stringify(drivers));localStorage.setItem('dogrh_driver_meta',JSON.stringify(driverMeta));localStorage.setItem('dogrh_races',JSON.stringify(races));localStorage.setItem('dogrh_teams',JSON.stringify(teams));localStorage.setItem('dogrh_season_state',JSON.stringify(seasonState));localStorage.setItem('dogrh_contracts',JSON.stringify(contracts));localStorage.setItem('dogrh_transfers',JSON.stringify(transferRecords));localStorage.setItem('dogrh_finance_budgets',JSON.stringify(financeBudgets));localStorage.setItem('dogrh_finance_cap_usage',JSON.stringify(financeCapUsage));localStorage.setItem('dogrh_sponsor_payments',JSON.stringify(sponsorPayments));localStorage.setItem('dogrh_finance_transactions',JSON.stringify(financeTransactions));localStorage.setItem('dogrh_finance_opening',JSON.stringify(financeOpeningBalances));localStorage.setItem('dogrh_finance_tx_overrides',JSON.stringify(financeTxOverrides));localStorage.setItem('dogrh_driver_finance_opening',JSON.stringify(driverFinanceOpeningBalances));localStorage.setItem('dogrh_driver_finance_opening_dates',JSON.stringify(driverFinanceOpeningDates));localStorage.setItem('dogrh_loan_agreements',JSON.stringify(loanAgreements));localStorage.setItem('dogrh_driver_licenses',JSON.stringify(driverLicenses))} ensureRaceMetadata(); load();
// 01/26 was only the old test/transition season and is permanently removed from this app build.
Object.keys(races).forEach(k=>{if(races[k]?.season==='01/26')delete races[k]});
contracts=contracts.filter(c=>c.season!=='01/26');
financeTransactions=financeTransactions.filter(t=>t.season!=='01/26');
transferRecords=transferRecords.filter(t=>t.season!=='01/26');
loanAgreements=loanAgreements.filter(t=>t.season!=='01/26');
delete financeBudgets['01/26']; delete financeCapUsage['01/26']; delete sponsorPayments['01/26']; delete financeOpeningBalances['01/26']; delete driverFinanceOpeningBalances['01/26']; delete driverFinanceOpeningDates['01/26'];
if(seasonState.current==='01/26')seasonState.current='02/26';
localStorage.setItem('dogrh_races',JSON.stringify(races));
localStorage.setItem('dogrh_contracts',JSON.stringify(contracts));
localStorage.setItem('dogrh_finance_transactions',JSON.stringify(financeTransactions));
localStorage.setItem('dogrh_transfers',JSON.stringify(transferRecords));
localStorage.setItem('dogrh_loan_agreements',JSON.stringify(loanAgreements));
// v1.87: offizieller Neustart der Rennsaison. Alle bisher mitgelieferten/gespeicherten Rennen werden einmalig entfernt.
if(localStorage.getItem('dogrh_race_reset_v187')!=='1'){
 Object.keys(races).forEach(k=>delete races[k]);
 financeTransactions=financeTransactions.filter(t=>t.type!=='driver_points_cost' && t.type!=='sponsor_race');
 Object.keys(sponsorPayments).forEach(k=>{if(sponsorPayments[k]?.type==='race')delete sponsorPayments[k]});
 seasonState.lastCalculatedAt=''; seasonState.lastResultCount=0; seasonState.lastRaceCount=0; seasonState.lastWeekendCount=0;
 localStorage.setItem('dogrh_race_reset_v187','1');
}
...
function showView(id){document.querySelectorAll('.view').forEach(v=>v.classList.remove('active'));document.getElementById(id).classList.add('active');document.querySelectorAll('.nav').forEach(n=>n.classList.toggle('active',n.dataset.view===id));document.getElementById('page-title').textContent={dashboard:'Dashboard',drivers:'Fahrer',compare:'Vergleich',teams:'Teams',races:'Rennen',wm:'Fahrer-WM',kwm:'KWM',archive:'Archiv'}[id]||'DoG RaceHub';if(id==='drivers')initDriverOverview();if(id==='compare')renderCompare();if(id==='teams')renderTeams();if(id==='races')initRaceSelectors();if(id==='wm')renderWM();if(id==='kwm')renderKWM();if(id==='dashboard')renderDashboard();if(id==='archive')renderArchive();updateStats()}
...
function saveTeamEditor(teamName){
 ...
 t.chief=document.getElementById('team-chief-edit').value.trim()||'ohne Teamchef';t.d1=d1;t.d2=d2;
...
function editRow(x,i,raceId){return `<div class="edit-grid" data-i="${i}"><label>Pos.<input class="e-pos" type="number" min="1" max="99" value="${i+1}"></label><label>Fahrer<input class="e-name" value="${esc(normDriver(x.name))}"></label><label>Team<input class="e-team" value="${esc(x.team)}"></label><label>Startposition<input class="e-grid" type="number" min="1" max="99" value="${x.grid}"></label><label>Zeit<input class="e-time" value="${esc(x.time)}"></label><label>Zeitstrafe (Sek.)<input class="e-pen" type="number" min="0" value="${x.penSec||0}"></label><label>TL<input class="e-tl" type="number" min="0" value="${x.tl||0}"></label><label>Nachträgliche Strafe<input class="e-penalty-note" value="${esc(x.penaltyNote||'')}" placeholder="z. B. 5 Sek. Zeitstrafe / Verwarnung"></label><label>Strafenbild<input class="e-penalty-image" type="file" accept="image/*" onchange="attachPenaltyImage(this,'${raceId}',${i})"></label><label>Status<select class="e-status"><option value="RESULT" ${x.status==='RESULT'?'selected':''}>Ergebnis</option><option value="DNF" ${x.status==='DNF'?'selected':''}>DNF</option><option value="DSQ" ${x.status==='DSQ'?'selected':''}>DSQ</option></select></label></div>`}
...
function syncDriverContractFinance(c){
 ...
 const specialConditional=/(beim|bei|wenn|sobald|ab)\s*(dem\s+)?erreichen|erreichen\s+von|bei\s+erreichen/i.test(agreementText);
 const specs=[
...
function openContractEditor(name,id){
 if(!requireEditor())return; const existing=id?contracts.find(c=>c.id===id):null; window.__editingContractId=existing?.id||''; const selectedDriver=existing?.driver||name||''; const selectedTeam=existing?.team||driverTeam(name)||''; const currentSeason=existing?.season||seasonState.current||'02/26'; const chief=existing?.teamChief||teams.find(t=>t.name===selectedTeam)?.chief||'';
 const seasonOpts=Object.keys(SEASON_META.seasons).sort().map(x=>`<option ${x===currentSeason?'selected':''}>${esc(x)}</option>`).join(''); const driverOpts=drivers.slice().sort((a,b)=>normDriver(a).localeCompare(normDriver(b),'de')).map(x=>`<option value="${esc(x)}"></option>`).join(''); const teamOpts=TEAM_CHOICES.filter(Boolean).map(x=>`<option value="${esc(x)}" ${x===selectedTeam?'selected':''}>${esc(x)}</option>`).join('');
 document.getElementById('modal-content').innerHTML=`<div class="modal-head"><div><h2>📄 ${existing?'Vertrag bearbeiten':'Fahrervertrag anlegen'}</h2><p class="race-edit-note">Vertragslayout nach deinem DoG-Beispielvertrag.</p></div><button onclick="closeModal()">×</button></div><div class="contract-form"><div class="contract-form-section"><h3>VERTRAGSPARTEIEN</h3><div class="new-driver-form"><label>Fahrer<input id="contract-driver" list="contract-driver-list" value="${esc(selectedDriver)}" placeholder="Fahrer auswählen oder neu eingeben"><datalist id="contract-driver-list">${driverOpts}</datalist></label><label>Saison<select id="contract-season">${seasonOpts}</select></label><label>Team<select id="contract-team" onchange="syncContractChief()">${teamOpts}</select></label><label>Division<select id="contract-div"><option ${existing?.division==='Div 1'||(!existing&&currentDivision(selectedDriver)==='Div 1')?'selected':''}>Div 1</option><option ${existing?.division==='Div 2'||(!existing&&currentDivision(selectedDriver)!=='Div 1')?'selected':''}>Div 2</option></select></label><label>Teamchef<input id="contract-chief" value="${esc(chief)}"></label></div></div><div class="contract-form-section"><h3>VERTRAGSLAUFZEIT</h3><div class="contract-season-lock"><b>Gesamte Saison ${esc(currentSeason)}</b><span>Ein normaler Fahrervertrag gilt immer für die komplette gewählte Saison. Rennrunden werden hier nicht erfasst.</span></div><div class="new-driver-form"><label>Beginn<input id="contract-start-date" type="date" value="${esc(existing?.startDate||'')}"></label><label>Ende<input id="contract-end-date" type="date" value="${esc(existing?.endDate||'')}"></label></div></div><div class="contract-form-section"><h3>FAHRERSPONSOR</h3><div class="new-driver-form"><label>Sponsor<select id="contract-sponsor" onchange="syncSponsorFields()"><option value="">Kein Sponsor</option>${Object.keys(SPONSOR_OPTIONS).map(x=>`<option value="${esc(x)}" ${existing?.sponsor===x?'selected':''}>${esc(x)}</option>`).join('')}</select></label><label>Sponsor-Logo (optional)<input id="contract-sponsor-logo" value="${esc(existing?.sponsorLogo||'')}" placeholder="Bildpfad oder URL"></label></div><div id="sponsor-preview" class="sponsor-preview"></div></div><div class="contract-form-section"><h3>ZAHLUNGSVEREINBARUNGEN</h3><div class="new-driver-form"><label>Gehalt<input id="contract-salary" value="${esc(existing?.salary||existing?.value||'')}" placeholder="4.000.000 €"></label><label>Sonderzahlung<input id="contract-special" value="${esc(existing?.specialPayment||'')}" placeholder="6.000.000 €"></label><label>Abfindung<input id="contract-severance" value="${esc(existing?.severance||'')}" placeholder="8.000.000 €"></label><label>Ausstiegsklausel<input id="contract-release" value="${esc(existing?.releaseClause||'')}" placeholder="120.000.000 €"></label></div></div><div class="contract-form-section"><h3>SONDERVEREINBARUNGEN</h3><label class="full-field">Vereinbarung<textarea id="contract-special-agreement" rows="4" placeholder="z. B. Sonderzahlung wird beim Erreichen von 180 Punkten ausgezahlt.">${esc(existing?.specialAgreement||'')}</textarea></label></div><div class="contract-form-section"><h3>📷 VERTRAGSBILD / OCR</h3><p class="race-edit-note">Lade den unterschriebenen Vertrag als Bild hoch. RaceHub liest den Vertragstext aus und versucht die Felder zuzuordnen. Alle Werte bleiben vor dem Speichern bearbeitbar.</p><input id="contract-image-input" type="file" accept="image/*" multiple onchange="attachContractImages(this)"/><div id="contract-image-preview" class="contract-image-preview">${(existing?.contractImages||[]).map(x=>`<img src="${esc(x)}" onclick="viewImage(this.src)">`).join()}</div><button class="ghost" type="button" onclick="document.getElementById('contract-ocr-input').click()">🔎 Vertrag per OCR auswerten</button><input id="contract-ocr-input" type="file" accept="image/*" multiple hidden onchange="startContractOCR(this)"/></div><div class="contract-form-section"><h3>UNTERSCHRIFTEN</h3><div class="new-driver-form"><label>Unterschrift Fahrer<input id="contract-sign-driver" value="${esc(existing?.signatureDriver||'')}" placeholder="Name / Signatur"></label><label>Unterschrift Teamchef<input id="contract-sign-chief" value="${esc(existing?.signatureChief||'')}" placeholder="Name / Signatur"></label></div></div></div><div class="modal-actions"><button class="ghost" onclick="closeModal()">Abbrechen</button><button class="primary" onclick="saveContract('${esc(existing?.id||'')}')">✓ Vertrag speichern</button></div>`;openModal();syncSponsorFields();
}
...
function saveContract(existingId){
 if(!requireEditor())return; const driver=document.getElementById('contract-driver').value.trim(),season=document.getElementById('contract-season').value,team=document.getElementById('contract-team').value,division=document.getElementById('contract-div').value; const startDate=document.getElementById('contract-start-date').value,endDate=document.getElementById('contract-end-date').value;
 if(!driver||!team){toast('Fahrer und Team sind erforderlich.');return} if(startDate&&endDate&&endDate<startDate){toast('Vertragsende darf nicht vor Vertragsbeginn liegen.');return} const linkedDriver=ensureDriverFromContract(driver,team,division);
 const existing=existingId?contracts.find(c=>c.id===existingId):null;
 const payload={driver:normDriver(linkedDriver||driver),season,team,division,teamChief:document.getElementById('contract-chief').value.trim(),startDate,endDate,sponsor:document.getElementById('contract-sponsor').value.trim(),sponsorLogo:document.getElementById('contract-sponsor-logo').value.trim(),salary:document.getElementById('contract-salary').value.trim(),specialPayment:document.getElementById('contract-special').value.trim(),severance:document.getElementById('contract-severance').value.trim(),releaseClause:document.getElementById('contract-release').value.trim(),specialAgreement:document.getElementById('contract-special-agreement').value.trim(),signatureDriver:document.getElementById('contract-sign-driver').value.trim(),contractImages:existing?.contractImages||[],signatureChief:document.getElementById('contract-sign-chief').value.trim(),value:document.getElementById('contract-salary').value.trim(),updatedAt:new Date().toISOString()};
 payload.id=existingId||''; const conflicts=contracts.filter(c=>c.id!==existingId && c.status!=='Beendet' && !c.draft && contractOverlap(payload,c));
 if(conflicts.length){const c=conflicts[0];toast(`⚠️ Vertragsüberschneidung: ${c.team} · ${c.season} · ${c.division}`);return;}
 const idx=contracts.findIndex(c=>c.id===existingId); if(idx>=0){payload.id=existingId;payload.createdAt=contracts[idx].createdAt;contracts[idx]={...contracts[idx],...payload};}else{payload.id=crypto.randomUUID?crypto.randomUUID():'contract-'+Date.now();payload.createdAt=new Date().toISOString();contracts.push(payload)} syncDriverContractFinance(payload); repairContracts(); save(); closeModal(); renderTeams(); initDriverOverview(); renderFinance(); openDriver(payload.driver); toast(existingId?'Vertrag aktualisiert und Fahrerfinanzen angepasst.':'Vertrag gespeichert und Fahrerfinanzen angelegt.');
}
...
let editor=true;function isEditor(){return editor} function openAdmin(){if(editor){adminPanel();return}document.getElementById('modal-content').innerHTML=`<div class="modal-card small"><div class="modal-head"><h2>Bearbeitung freischalten</h2><button onclick="closeModal()">×</button></div><p class="race-edit-note">Alle dürfen alles sehen. Nur freigeschaltete Bearbeiter dürfen Daten ändern.</p><input id="admin-pin" type="password" placeholder="Liga-Leiter / Editor-Code"><div class="modal-actions"><button class="ghost" onclick="closeModal()">Abbrechen</button><button class="primary" onclick="unlock()">Freischalten</button></div></div>`;openModal()}
function unlock(){let pin=document.getElementById('admin-pin').value,saved=localStorage.getItem('dogrh_pin');if(!saved){if(!pin){toast('Bitte einen Liga-Leiter-Code festlegen.');return}localStorage.setItem('dogrh_pin',pin);saved=pin}if(pin===saved){editor=true;closeModal();toast('Bearbeitung freigeschaltet.')}else toast('Code ist nicht korrekt.')}
function requireEditor(){if(!editor){openAdmin();return false}return true} function adminPanel(){const q=databaseIntegrity();document.getElementById('modal-content').innerHTML=`<div class="modal-head"><h2>DoG Liga-Verwaltung</h2><button onclick="closeModal()">×</button></div><p class="race-edit-note">Der Liga-Leiter bestimmt, wer bearbeiten darf.</p><div class="admin-integrity"><b>Datenstatus</b><span>${q.races} Rennen · ${q.weekends} Rennwochenenden · ${q.results} Ergebnisse</span><span>${q.orphan?'⚠ '+q.orphan+' unbekannte Fahrer':'✓ Fahrerzuordnungen OK'}</span><span>${q.duplicate?'⚠ '+q.duplicate+' doppelte Ergebniszeilen':'✓ Keine doppelten Ergebniszeilen'}</span><span>${q.invalid?'⚠ Punkte prüfen':'✓ Punkte konsistent'}</span></div><button class="primary" onclick="recalculateDatabase()">🔄 Gesamte Datenbank neu berechnen</button><button class="ghost" onclick="resetPin()">Liga-Leiter-Code ändern</button><button class="ghost" onclick="editor=false;closeModal();toast('Bearbeitung gesperrt.')">🔒 Bearbeitung sperren</button>`;openModal()} function resetPin(){const p=prompt('Neuen Liga-Leiter-Code eingeben:');if(p){localStorage.setItem('dogrh_pin',p);toast('Code geändert.')}}
...
updateSeasonChrome();showView('dashboard');
